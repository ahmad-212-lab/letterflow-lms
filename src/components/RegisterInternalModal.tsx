...
export default function RegisterInternalModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setIsOpen(false);
        setFormData({
          referenceNo: "", title: "", sender: "Human Resources", recipient: "IT Department", description: "", priority: "High"
        });
        setFile(null);
        router.refresh();
      } else {
        const errorData = await res.json();
        alert(errorData.error || "Failed to register internal memo");
      }
    } catch (error) {
      console.error(error);
      alert("Error registering memo");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <button className="btn btn-primary" onClick={() => setIsOpen(true)}>
        <Plus size={18} style={{ marginRight: '0.5rem' }}/> New Internal Memo
      </button>
      ...
      {isOpen && (
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: '500px', position: 'relative' }}>
            ...
            <h3 style={{ marginBottom: '1.5rem' }}>Create Internal Memo</h3>
            
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.875rem', marginBottom: '0.5rem', fontWeight: 500 }}>Memo Ref No.</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.875rem', marginBottom: '0.5rem', fontWeight: 500 }}>From Dept.</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.875rem', marginBottom: '0.5rem', fontWeight: 500 }}>To Dept.</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', marginBottom: '0.5rem', fontWeight: 500 }}>Memo Content</label>
                <textarea required name="description" value={formData.description} onChange={handleChange} rows={3} style={{ width: '100%', padding: '0.5rem', borderRadius: '0.25rem', border: '1px solid var(--border-color)', resize: 'none', marginBottom: '1rem' }}></textarea>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', marginBottom: '0.5rem', fontWeight: 500 }}>Attach File (Optional)</label>
                <input 
                  type="file" 
